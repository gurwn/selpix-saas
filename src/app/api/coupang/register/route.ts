...
// NOTE: 현재 하드코딩된 쿠팡 인증 정보입니다. 운영/배포 전 반드시 .env로 옮기고 git에 노출되지 않도록 하세요.
...
const USER_ID = 'deadrow' // Wing 로그인 ID
  ...
  try {
    ...
    const init: RequestInit = {
      ...
      headers: {
        ...
        'X-Requested-By': VENDOR_ID // V5 API에서는 필수일 수 있음
      },
    }
    if (body && method !== 'GET') {
...
async function fetchCategoryMeta(displayCategoryCode?: string | number) {
  ...
  try {
    const { json, res } = await coupangFetch('GET', path)
    if (!res.ok || json?.code === 'ERROR') {
      console.warn('[coupang-meta-error]', { status: res.status, body: json })
      return null
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (!outRes.ok) {
      ...
      const outCreate = await coupangFetch('POST', outboundPath, outboundBody)
    ...
    // --- 반품지: 고정 이름으로 조회만 수행 (생성하지 않음) ---
    let retRes: any = { ok: false, status: 0 }
    let retJson: any = {}
    try {
      const { res: listRes, json: listJson } = await coupangFetch('GET', returnListPath, undefined, 'pageNum=1&pageSize=50')
      ...
      if (target?.returnCenterCode) {
        ...
        return NextResponse.json({
          ...
          message: `반품지(${FIXED_RETURN_NAME}) 코드가 없습니다. 쿠팡 Wing에서 반품지를 생성 후 다시 시도하세요.`,
          outbound: {
            success: outRes.ok,
            code: outJson?.data?.outboundShippingPlaceCode || outJson?.data?.shippingPlaceId
          }
    ...
    if (payload.productName && retRes.ok && outRes.ok) {
      try {
        if (!outboundShippingPlaceCode) {
        ...
        if (payload.mainImage) {
          images.push({
            ...
            vendorPath: payload.mainImage
          })
        }

        const itemPrice = Number(payload.sellPrice) || 0
        const itemName = `${payload.productName}_1`
        ...
        // 카테고리 코드 정규화 (공백/빈값 -> undefined)
        const incomingCategory = (payload.categoryCode ?? payload.displayCategoryCode ?? '').toString().trim()
        const normalizedCategoryCode = incomingCategory === '' ? undefined : incomingCategory
        ...
        const productPayload = {
          ...
          sellerProductName: payload.sellerProductName || payload.productName, // 내부 관리용 (도매꾹 원래 이름 등)
          displayProductName: payload.productName, // 노출용 상품명 (AI 추천 등)
          generalProductName: payload.productName, // 검색용 상품명
        ...
        const { res: prodRes, json: prodJson } = await coupangFetch('POST', productPath, productPayload)
    ...
    // 결과 응답
    return NextResponse.json({
      ok: outRes.ok && retRes.ok,
      message: "생성 요청 완료 (상세 결과 확인 필요)",
      outbound: {
        success: outRes.ok,
        data: outJson,
